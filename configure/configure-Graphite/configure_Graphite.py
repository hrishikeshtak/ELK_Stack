...
class Graphite():
    ...
    def validateAllParams(self, config_file):
        ...
        for section in sectionToBeValidate:
            ...
            if section == "GRAPHITE":
                optionList = ['enable_graphite',
                              'graphite_local_ipaddr', 'graphite_web_port']
                ...
                if res == 0:
                    for paramater in optionList:
                        if paramater == "enable_graphite":
                            ...
                            GRAPHITE_LOCAL_IPADDR = config.get(
                                section, paramater)
    ...
    def configure_graphite(self):
        ...
        self.execute_command(
            command, None, "configure Graphite not successful")
        ...
        self.execute_command(
            command, DEVNULL, "configure Graphite not successful")
        command = "echo \"TIME_ZONE = 'Asia/Kolkata'\" | sudo tee --append %s/local_settings.py;" % GRAPHITE_CONF_DIR
        self.execute_command(
            command, DEVNULL, "configure Graphite not successful")
        command = "echo \"ALLOWED_HOSTS = ['localhost', '%s']\" | sudo tee --append %s/local_settings.py;" % (
            GRAPHITE_LOCAL_IPADDR, GRAPHITE_CONF_DIR)
        self.execute_command(
            command, DEVNULL, "configure Graphite not successful")
        command = "sudo sed -i -e '/USE_REMOTE/ s/^#//' %s/local_settings.py;" % GRAPHITE_CONF_DIR
        self.execute_command(
